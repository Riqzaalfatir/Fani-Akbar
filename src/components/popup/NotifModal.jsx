...
const IconCheck = () => (
  <img src="/images/popup/CeklisCircle.png" alt="check" className="w-12 h-12 lg:w-16 lg:h-16 object-contain" />
)
const IconWarning = () => (
  <img src="/images/popup/TandaSeru.png" alt="warning" className="w-12 h-12 lg:w-16 lg:h-16 object-contain" />
)
const IconQuestion = () => (
  <img src="/images/popup/TandaTanya.png" alt="question" className="w-12 h-12 lg:w-16 lg:h-16 object-contain" />
)
...
const NotifModal = ({ type, onClose, onConfirm, waNumber = "6281234567890" }) => {
  ...
  return (
    ...
    >
      {/* 2. CARD */}
      <div className="relative bg-white rounded-lg shadow-xl flex flex-col items-center text-center animate-modalIn overflow-hidden w-[322px] h-[360px] lg:w-[485px] lg:h-[543px]">
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-4 w-full">
          ...
          {/* Title */}
          <h3 className="uppercase tracking-widest mb-4 lg:mb-6 text-[16px] lg:text-[24px] font-extrabold text-[#605F5E] font-noto"
            style={{ fontFamily: "'Noto Sans', sans-serif", letterSpacing: '0.12em' }}
          >
            {config.title}
          </h3>
        ...
        <div className="w-full flex flex-col h-[52px] lg:h-[78px]">
          <div className="w-full h-px bg-[#605F5E]/30" />
          <div className="flex flex-row flex-1 w-full">
            {config.buttons.map((btn, i) => (
              <button
                key={i}
                onClick={() => handleAction(btn.action)}
                className="flex-1 flex items-center justify-center gap-1.5 transition-colors duration-150 text-[14px] lg:text-[21px]"
                style={{
                  fontFamily: "'Noto Sans', sans-serif",
                  color: btn.variant === 'whatsapp' ? '#605F5E' : btn.variant === 'solid' ? '#605F5E' : '#605F5E',
                  fontWeight: btn.variant === 'outline' ? 400 : 700,
      ...
      {/* 5. STYLE - di luar card, di dalam backdrop */}
      <style jsx global>{`
